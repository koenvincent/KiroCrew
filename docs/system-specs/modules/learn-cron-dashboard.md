...
store — the final check and the unlink run inside that store's lock through
`state.read_folders`, so a concurrent pin cannot slip between them), webhook
tokens (`agent` in the token store) and private copies (`forked_from` in the
fork sidecar), and answering with the reference LIST rather than prose so the
tab can offer a jump to each. A package, runtime, markdown or private-copy spec is
