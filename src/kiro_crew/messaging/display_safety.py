...
def safe_resume_offset(
    frozen_head: str, text: str, limit: int, redactor: Callable[[str], str]
) -> int:
    """Where *text* may RESUME below a head that is already on screen.

    The sibling of :func:`safe_split_offset`, for the case where one side of the
    boundary cannot be rewritten. :func:`safe_split_offset` moves the head
    WITH the cut, which is right when both pieces are being created now. Here the
    head is *frozen_head* -- text a previous delivery already showed -- so moving
    the resume offset back does not shorten it. Grading a candidate as
    ``text[:candidate]`` against ``text[candidate:]`` would then judge a head the
    reader never saw, and the pair actually on screen would go unchecked.

    That distinction is the whole bug this exists for: an answer can carry a key's
    tail EARLIER than the frozen boundary and the key's head right at it. Each
    delivery is clean scanned alone, the partition check passes at a moved-back
    offset, and the two deliveries read one after the other still spell the key.

    Candidates step back exponentially from ``min(limit, len(text))``, the same
    ladder and the same cost bound as :func:`safe_split_offset`, and ``0`` -- resume
    from the very beginning, re-sending everything -- is tried last.

    When no sampled candidate is safe, including ``0``, the return is ``len(text)``:
    resume after everything, i.e. deliver nothing yet. Withholding is always safe
    and always available, because a tail that is not shown cannot join *frozen_head*
    into anything. It is also the only remedy left, since the head cannot be edited.
    """
    ...
    offset, step = base, 0
    while True:
        if not joins_to_a_credential(frozen_head, text[offset:], redactor):
            return offset
        if offset == 0:
            return len(text)
        step = 1 if step == 0 else step * 2
        offset = max(0, base - step)
