...
| Agent template (`kiro_agent`) | The spec it boots from — tools, MCP servers, instructions | the default `kirocrew` spec |
| Workspace | The folder it reads and writes | the `default` workspace |
...
| Reasoning effort | How long its sessions think before answering | the global default; ignored by models that do not reason |
...
the welcome screen, the schedule form, the Projects task runner, an agent
channel, and — per thread — Slack's `!ta <name>` (`!ta off` clears it). Kiro
Crew remembers the crewmate a session committed to across a restart, so a
crewmate pick stays a crewmate pick and does not decay into a bare template.
...
`select_crew(crew="<name>")` binds one and returns what it resolved to —
`kiro_agent`, `workspace`, `memory_store`, `model` — and records the routing
decision in that crewmate's activity log.
