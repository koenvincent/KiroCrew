...
- **The `send_message` agent tool reaches Teams two ways**: `session="teams"` sends a
  DM to this channel's own configured owner, and `channel_type="teams"` posts into the
  conversation the turn is already running in. A cron result also arrives here when its
  originating dashboard chat is mirrored into this conversation (that is what `/link`
  binds). What is Slack-only is the `channel`, `user`, `blocks` and `thread_ts` options —
  their addressing, allow-list and threading are Slack concepts, and combining them with
  a Teams target is refused rather than ignored.
