...
def _open_legacy_regular_file(path: Path) -> TextIO | None:
    ...
    * a symlink redirects the read somewhere the member directory does not own.
    ...
    ``O_NONBLOCK`` makes the FIFO open return at once instead of waiting, and the
    ``fstat`` then refuses it for what it is; ``O_NOFOLLOW`` refuses the symlink
    in the kernel. Both are feature-detected rather than platform-detected, so a
    platform lacking either keeps the regular-file check, which is the part that
    decides. On a regular file ``O_NONBLOCK`` changes nothing about the reads.

    ``None`` is the same answer an absent file gives: migrate nothing from it.
    """
...
class MemberEventLogService:
    ...
    def append_closer_if_still_applies(
        ...
        data: dict,
        *,
        still_applies: Callable[[dict], bool],
    ) -> Event | None:
        ...
        with lock:
            ...
            if not still_applies(values if isinstance(values, dict) else {}):
                return None
            return self._append_locked(slug, log, type, data)
