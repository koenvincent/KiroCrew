...
class TestAppSecretsAreHiddenFromScriptCrons:
    ...
    def test_every_app_secret_is_masked_on_the_spawn(self, home, monkeypatch, granted):
        ...
        # Asserted as CONTAINMENT, not an exact leaf spelling. The mask names the
        # apps TREE, which covers these three and every app installed after this
        # spawn -- the property a leaf list read out of the tree cannot have,
        # because a live child's masks are never recomputed.
        assert str(home / "apps") in seen["hidden"]
        for s in secrets:
            assert any(
                Path(s).is_relative_to(Path(h)) for h in seen["hidden"]
            ), f"{s} not masked (granted={granted})"

    def test_the_bundle_tree_is_masked_with_a_window_for_the_script(self, home, monkeypatch):
        """Replaces an earlier pin that asserted the apps tree stayed VISIBLE.

        That pin encoded masking LEAVES, which cannot cover an app installed while a
        long-running script cron is still executing. The tree is masked instead, and
        the one directory the launcher must import from is re-exposed as a private
        window -- ``sandbox._private_window_spellings`` admits only a proper
        descendant of a hidden directory and never lifts the parent's mask, so every
        sibling app stays hidden while the script stays importable.
        """
        import kiro_crew.cron_script as cs
        from kiro_crew.sandbox import SandboxUnavailableError
        ...
        def _wrap(argv, mode="", extra_hidden_dirs=(), extra_private_dirs=(), **kw):
            seen["hidden"] = tuple(extra_hidden_dirs)
            seen["private"] = tuple(extra_private_dirs)
            raise SandboxUnavailableError("captured", "test", "spawn argument captured")
        ...
        assert str(home / "apps") in seen["hidden"]
        assert seen["private"] == (str(script.parent.resolve()),)

    def test_a_nested_bundle_script_also_keeps_its_apps_durable_data_dir(self, home, monkeypatch):
        """The two script spellings are not equivalent, and only one is covered for free.

        A script at the bundle ROOT gets a window at that root, which already
        contains ``data/``. The nested ``crons/job.py`` spelling gets a window
        BESIDE it, so ``app_data_dir`` would resolve into the masked tree and every
        durable write would be discarded at exit -- silently, and for the layout the
        window docstring recommends. Both windows must therefore reach the spawn,
        which is what pins the call site rather than the helper alone.
        """
        import kiro_crew.cron_script as cs
        ...
        self._installed(home, APP)
        script = _bundle(home) / "crons" / "job.py"
        ...
        cs.run_script_sandboxed(f"{script}:run", "job-data", "msg")

        data = str(app_data_dir(APP).resolve())
        assert str(home / "apps") in seen["hidden"]
        assert seen["private"] == (str(script.parent.resolve()), data)
        # The point of the pairing: the app's OWN secret is still not in either
        # window, so covering its data did not cost the property the mask buys.
        own_secret = (_bundle(home) / ".app_secret").resolve()
        for window in seen["private"]:
            assert not own_secret.is_relative_to(Path(window))

    def test_a_granted_run_takes_no_window(self, home, monkeypatch):
        """A grant's isolation must not be traded away to keep a bundle importable.

        The granted branch deliberately hides the script's own parent to close the
        mutable-sibling import route, and its verified body travels over stdin so the
        child never reads the file. Re-exposing that directory as a window would
        reopen exactly that route, so a granted run gets none.

        Driven with a ``crons/`` script because that is the only granted case that
        REACHES the spawn: the grant reader (``_read_script_body``) is pinned to
        ``crons/``, so a granted bundle script is refused before it gets here.
        """
        import kiro_crew.cron_script as cs
        from kiro_crew.sandbox import SandboxUnavailableError
    ...
    def test_the_spawn_refuses_when_the_credential_mask_cannot_be_established(
        self, home, monkeypatch
    ):
        """A control requested and not built refuses, rather than running unmasked.

        `_app_secret_masks` answers `None` when the apps tree cannot be created -- a
        data home that will not resolve, or a plain file squatting the name. Treating
        that as an empty mask would hand the child every installed app's bearer
        credential with nothing recording that the mask was skipped, so the spawn is
        refused before `wrap_argv` is reached at all.
        """
        import kiro_crew.cron_script as cs
        ...
        def _wrap(argv, mode="", **kw):
            reached["wrap"] = True
            raise AssertionError("wrap_argv must not be reached when the mask is refused")

        monkeypatch.setattr(cs, "wrap_argv", _wrap)
        monkeypatch.setattr(cs, "resolve_script_path", lambda spec, **_kw: (str(script), "run"))
        monkeypatch.setattr(cs, "_app_secret_masks", lambda: None)

        out = cs.run_script_sandboxed(f"{script}:run", "job-mask-refused", "msg")

        assert reached["wrap"] is False
        assert out["status"] == "error"
        assert "app-credential mask" in out["error"]
