...
class TestEgress:
    ...
    def test_an_unscreened_row_is_not_in_either_arm(self, home, bg_loop, enabled, monkeypatch):
        """A row the screen dropped was never offered.

        Counting it as something Jev declined would attribute a drop to an answer
        nobody asked for, so it is absent from the baseline as well as the pick --
        and the block keeps it, because the caller only ever removes.
        """
        ...
        assert _rows(home)[-1]["baseline_keys"] == ["a"]
...
class TestTheHookAndTheStoreAgreeOnIdentity:
    ...
    def test_a_copying_point_would_lose_every_decision(self, home, bg_loop, enabled, monkeypatch):
        ...
        original = mr.screened_rows
        with patch.object(mr, "screened_rows", lambda c, s: [dict(row) for row in original(c, s)]):
            hook = mr.keep_hook("hi", session_key="s", loop=bg_loop, owner_turn=True)
