...
class TestARootSwappedBetweenTheTwoReplacePhasesIsRefused:
    """`_refuse_unsafe_destination_roots` runs before phase one; every rollback copy is
    taken between it and the mutation loop. A root swapped for a link in that window was
    screened by nothing, and the mutation loop deletes before it writes."""

    @requires_symlinks
    def test_the_mutation_loop_refuses_and_writes_nothing_outside_the_home(
        self, tmp_path, monkeypatch, capsys
    ):
        import shutil
        ...
        real = snap._backup_tree_or_refuse

        def _swap_after_backup(src, dst, **kwargs):
            real(src, dst, **kwargs)
            if src.name == "artifacts":
                shutil.rmtree(src)
                src.symlink_to(elsewhere, target_is_directory=True)

        monkeypatch.setattr(snap, "_backup_tree_or_refuse", _swap_after_backup)
        rc = _restore(bundle, "replace", "artifacts")
        out = capsys.readouterr().out
        assert rc != 0, out
        assert "stopped resolving inside the data home" in out, out
        assert not (elsewhere / "report.md").exists(), "the restore wrote outside the data home"


class TestANonDirectoryAtATreeRootIsRefusedBeforeAnythingMoves:
    """`safe_tree_root` answers containment, not type, so a regular FILE standing at a
    tree's name passed every check. What followed cost the operator that file: the
    rollback pass saves a root only when it is a directory, the mutation records the name
    as reached before it writes, `must_create` refuses the occupied name, and recovery
    then unlinks an occupant nothing ever saved. Refused up front instead."""

    def _file_at_root(self, home: Path, name: str) -> Path:
        import shutil

        shutil.rmtree(home / name)
        target = home / name
        target.write_text("the only copy")
        return target

    def test_replace_refuses_and_the_file_survives(self, tmp_path, monkeypatch, capsys):
        home = _home(tmp_path, monkeypatch)
        bundle = _snapshot(tmp_path)
        victim = self._file_at_root(home, "uploads")

        rc = _restore(bundle, "replace", "uploads")
        printed = capsys.readouterr().out
        assert rc != 0, printed
        assert "not directories" in printed, printed
        assert victim.read_text() == "the only copy", "the only copy was deleted"

    def test_merge_refuses_it_too(self, tmp_path, monkeypatch, capsys):
        """Merge destroys nothing, but it would create the tree THROUGH that name and
        report an import that did not happen, so it refuses on the same check. Driven on
        `skills` because the check sits at the shared chokepoint and `artifacts`/`uploads`
        meet the replace-only refusal first."""
        home = _home(tmp_path, monkeypatch)
        bundle = _snapshot(tmp_path)
        victim = self._file_at_root(home, "skills")

        rc = _restore(bundle, "merge", "skills")
        printed = capsys.readouterr().out
        assert rc != 0, printed
        assert "not directories" in printed, printed
        assert victim.read_text() == "the only copy"

    def test_nothing_was_saved_moved_or_removed(self, tmp_path, monkeypatch):
        """The refusal has to land before the rollback directory is even allocated:
        a refusal that leaves a half-populated `pre-restore-*` behind is a different
        outcome from one that changed nothing."""
        home = _home(tmp_path, monkeypatch)
        bundle = _snapshot(tmp_path)
        self._file_at_root(home, "uploads")

        assert _restore(bundle, "replace", "uploads") != 0
        assert not list(home.glob("pre-restore-*"))

    def test_an_ordinary_directory_root_is_still_accepted(self, tmp_path, monkeypatch):
