...
function pairsOf(
  ...
  if (schema?.path?.length) {
    ...
    // Sliced like the list branch above, and for the same two reasons. The cap is
    // what keeps one contributor's card from filling the drawer, and `MoreRows`
    // draws from `total`, so returning `pairs.length` as the total made the marker
    // describe a truncation that had not happened: 25 selectors rendered all 25 and
    // said "+5 more". The total stays the real count so the marker tells the truth.
    return { pairs: pairs.slice(0, MAX_ROWS), total: pairs.length }
  }
