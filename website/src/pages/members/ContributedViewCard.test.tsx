...
describe('a schema wider than the row cap', () => {
  it('renders at most the cap and counts the remainder truthfully', () => {
    // The list branch already slices and reports the real length. The schema branch
    // rendered EVERY selector while the more-marker, which reads the total, announced
    // a truncation that had not happened.
    const path = Array.from({ length: 25 }, (_, i) => `f${i}`)
    const value: Record<string, number> = {}
    path.forEach((k, i) => {
      value[k] = i
    })

    render(<ContributedViewCard view={view({ value, schema: { path } })} />)

    expect(screen.queryByText('f19')).not.toBeNull()
    expect(screen.queryByText('f20')).toBeNull()
  })
})
