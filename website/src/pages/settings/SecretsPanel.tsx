 ...
 *
 * `mc-auth-cleared` fires when the shared banner's token exchange succeeds. At
 * that moment a card reciting "Session expired. Run kirocrew token ... to sign
 * back in." points at a banner that is gone and a session that has been
 ...
 *
 * `isAuthExpiredError` is the whole point of the guard. The event says auth
 ...
 * true after an unrelated auth recovery elsewhere in the app, so resetting it
 * would delete a message the user has not acted on. Only a failure the event
 * actually resolves is cleared.
 *
 ...
 */
function useResetOnAuthCleared(onCleared: () => void): void {
  ...
  useEffect(() => {
    ...
    window.addEventListener('mc-auth-cleared', handle)
    return () => window.removeEventListener('mc-auth-cleared', handle)
  }, [])
}
